...
/** @file
* @brief
* @custom Thiago Passamani <thiagopassamani@gmail.com>
*/
...
//          $toadd['_no_notif'] = true;
   ...
   **/
   function numberOfFollowups($with_private=1) {
      ...
      //$query = "SELECT COUNT(*) // CUSTOM - Thiago Passamani
   ...
   **/
   function countActiveTicketsForItem($itemtype, $items_id) {
      ...
      //$query = "SELECT COUNT(*) AS cpt // CUSTOM - Thiago Passamani
   ...
   **/
   static function getAllStatusArray($withmetaforsearch=false) {
      ...
      $tab = array(self::INCOMING       => _x('status', 'New'),
                   ...
                   self::IN_ATTENDANCE  => _x('status', 'In Attendance'), /* Thiago Passamani - Em atendimento */
                   self::FEEDBACK       => _x('status', 'Feedback'), /* Thiago Passamani - Aguardando feedback */
                   self::WAITING        => __('Pending'),
                   self::WAITING_LIST   => _x('status', 'Waiting List'), /* Thiago Passamani - Lista de espera */
   ...
   **/
   ...
   /**
    * Get the ITIL object assign or plan status list // CUSTOM - Thiago Passamani
    *
    * @since version 0.83
    *
    * @return an array
   **/
   ...
   /* Thiago Passamani - Em atendimento */
   static function getInAttendanceStatusArray() {
      return array(self::IN_ATTENDANCE);
   }

   /* Thiago Passamani - Aguardando feedback */
   static function getFeedbackStatusArray() {
      return array(self::FEEDBACK);
   }

   /* Thiago Passamani - Lista de espera */
   static function getWaitingListStatusArray() {
      return array(self::WAITING_LIST);
   }

   /**
    * Get the ITIL object closed, solved, waiting list or waiting status list // CUSTOM - Thiago Passamani
    *
   ...
   **/
   static function getReopenableStatusArray() {
      return array(self::CLOSED, self::SOLVED, self::WAITING,self::WAITING_LIST);
   }
...
//       "<div id='uploadfiles'><input type='file' name='filename[]' value='' size='60'></div>";
   ...
   **/
   ...
   static function showCentralList($start, $status="process", $showgrouptickets=true) {
      ...
      switch ($status) {
         ...
         case "in_attendance" : /* Thiago Passamani - Em atendimento */
            $query .= "WHERE $is_deleted
                             AND ( $search_assign )
                             AND `status` = '".self::IN_ATTENDANCE."' ".
                             getEntitiesRestrictRequest("AND", "glpi_tickets");
            break;

         case "feedback" : /* Thiago Passamani - Aguardando feedback */
            $query .= "WHERE $is_deleted
                             AND ( $search_assign )
                             AND `status` = '".self::FEEDBACK."' ".
                             getEntitiesRestrictRequest("AND", "glpi_tickets");
            break;             
         ...
         case "waiting_list" : /* Thiago Passamani - Lista de espera */
            $query .= "WHERE $is_deleted
                             AND ( $search_assign )
                             AND `status` = '".self::WAITING_LIST."' ".
                             getEntitiesRestrictRequest("AND", "glpi_tickets");
            break;             
         ...
         if ($showgrouptickets) {
            switch ($status) {
               ...
               case "in_attendance" :
                  $options['criteria'][0]['field']      = 12; // status
                  $options['criteria'][0]['searchtype'] = 'equals';
                  $options['criteria'][0]['value']      = self::IN_ATTENDANCE;
                  $options['criteria'][0]['link']       = 'AND';
                  ...
                  echo "<a href=\"".$CFG_GLPI["root_doc"]."/front/ticket.php?".
                         Toolbox::append_params($options,'&amp;')."\">".
                         Html::makeTitle(__('Tickets in attendance status'), $number, $numrows)."</a>";
                  break;
               ...
               case "feedback" :
                  $options['criteria'][0]['field']      = 12; // status
                  $options['criteria'][0]['searchtype'] = 'equals';
                  $options['criteria'][0]['value']      = self::FEEDBACK;
                  $options['criteria'][0]['link']       = 'AND';
                  ...
                  echo "<a href=\"".$CFG_GLPI["root_doc"]."/front/ticket.php?".
                         Toolbox::append_params($options,'&amp;')."\">".
                         Html::makeTitle(__('Tickets on feedback status'), $number, $numrows)."</a>";
                  break; 
               ...
               case "waiting_list" :
                  $options['criteria'][0]['field']      = 12; // status
                  $options['criteria'][0]['searchtype'] = 'equals';
                  $options['criteria'][0]['value']      = self::WAITING_LIST;
                  $options['criteria'][0]['link']       = 'AND';
                  ...
                  echo "<a href=\"".$CFG_GLPI["root_doc"]."/front/ticket.php?".
                         Toolbox::append_params($options,'&amp;')."\">".
                         Html::makeTitle(__('Tickets on waiting list status'), $number, $numrows)."</a>";
                  break;                  
            ...
            switch ($status) {
               ...
               case "in_attendance" :
                  ...
                  $options['criteria'][1]['field']      = 12; // status
                  $options['criteria'][1]['searchtype'] = 'equals';
                  $options['criteria'][1]['value']      = self::IN_ATTENDANCE;
                  $options['criteria'][1]['link']       = 'AND';
               ...
               case "feedback" :
                  ...
                  $options['criteria'][1]['field']      = 12; // status
                  $options['criteria'][1]['searchtype'] = 'equals';
                  $options['criteria'][1]['value']      = self::FEEDBACK;
                  $options['criteria'][1]['link']       = 'AND';
   ...
   **/
   static function cronInfo($name) {
      ...
      switch ($name) {
         ...
         /* Thiago Passamani - Aguardando feedback (Fechamento automático) */  
         case 'closeticketfeedback' :
            return array('description' => __('Automatic tickets closing in feedback'));    

         /* Thiago Passamani - Retorno de Feedback do usuário */  
         case 'returnticketfeedback' :
            return array('description' => __('Return automatic of tickets when received user feedback.'));    
   ...
   **/
   static function cronCloseTicket($task) {
      ...
      foreach (Entity::getEntitiesToNotify('autoclose_delay') as $entity => $delay) {
         if ($delay >= 0) {
            ...
            // Thiago Passamani - Ordena pela ordem do mais antigo e limita a quantidade de registros conforme o $delay de fechamento.
            $query .= " ORDER BY `solvedate` ASC "; // Ordena pela data de solução
            $query .= " LIMIT ".$delay_solve."; ";  // $delay é o mesmo para chamado solucionados (default 10 tickets), 
                                                    // caso necessário substitua por [ $query .= " LIMIT 10; "; ] onde 10 é a 
                                                    // quantidade de chamados que será retornados para consulta.
            $nb = 0;
            foreach ($DB->request($query) as $tick) {
   ...
   /**
    * Cron for ticket's automatic close in Feedback - Thiago Passamani - Status: Aguardando feedback
    *
    * @param $task : crontaskfeedback object
    *
    * @return integer (0 : nothing done - 1 : done)
   **/
   static function cronCloseTicketFeedback($task) {
      global $DB;
      ...
      foreach (Entity::getEntitiesToNotify('autoclose_delay') as $entity => $delay) {
         if ($delay >= 0) {
            ...
            if ($delay > 0) {
               $query .= " AND ADDDATE(`date_mod`, INTERVAL ".$delay." DAY) < NOW() ";
            }

            $delay_feedback = $delay * 2;

            // Thiago Passamani - Ordena pela ordem do mais antigo e limita a quantidade de registros conforme o $delay de fechamento.
            $query .= " ORDER BY `date_mod` ASC "; // Ordena pela data de solução
            $query .= " LIMIT ".$delay_feedback."; ";              // Delay default 10 tickets
            ...
            foreach ($DB->request($query) as $tick) {
               ...
               if($tick['status'] == self::FEEDBACK)
               {
                  $query_feedback = "INSERT INTO `glpi_ticketfollowups` (`tickets_id`, `date`, `users_id`, `content`, `requesttypes_id`) VALUES ('".$tick['id']."', NOW(), 2, 'Chamado fechado automaticamente por decurso de prazo para o Feedback.', 1); ";
                  // EN //$query_feedback = "INSERT INTO `glpi_ticketfollowups` (`tickets_id`, `date`, `users_id`, `content`, `requesttypes_id`) VALUES ('".$tick['id']."', NOW(), 2, 'Chamado fechado automaticamente por decurso de prazo para o Feedback.', 1); ";

                  $DB->query($query_feedback);
               } 
      ...
      return ($tot > 0);
      /* 
      // Inserir essa no banco de dados:
      INSERT IGNORE INTO `glpi_crontasks` (`itemtype`, `name`, `frequency`, `param`, `state`, `mode`, `allowmode`, `hourmin`, `hourmax`, `logs_lifetime`, `lastrun`, `lastcode`, `comment`) VALUES ('Ticket', 'closeticketfeedback', 21600, NULL, 1, 1, 3, 0, 24, 30, '2013-12-09 16:02:00', NULL, NULL);
      */
   }
/**************************************************************************************************************************/
/**
    ...
    * @return integer (0 : nothing done - 1 : done)
   **/
   static function cronReturnTicketFeedback($task) {
      global $DB;
      ...
      /**
      Seleciona todos os chamados com status "FEEDBACK" (Aguardando Feedback) com o ultimo followup 'resposta' do usuário 
      e retorna para PROCESSADO ou EM ATENDIMENTO.
      */
      $query = "SELECT `gt`.`id`,
                       `gt`.`status`,
                       `gt`.`date_mod`,
                ...
                INNER JOIN `glpi_ticketfollowups` AS `gtf` ON(`gtf`.`tickets_id` = `gt`.`id`)
                INNER JOIN `glpi_users` AS `gu` ON(`gu`.`id` = `gtf`.`users_id`)
                INNER JOIN `glpi_tickets_users` AS `gtu` ON(`gtu`.`users_id` = `gu`.`id`)
                WHERE `gt`.`status` = '".self::FEEDBACK."'
                      AND `gt`.`is_deleted` = 0 
                      AND `gtf`.`date` = `gt`.`date_mod`
                      AND `gtu`.`type` = 1
                      AND `gtu`.`tickets_id` = `gt`.`id` ";
                      //`gt`.`entities_id` = '".$entity."'

                      /**
                      Verificar de gtu.type IN(1,2) ( requerente e observador)
                      */
      ...
      foreach ($DB->request($query) as $tick)
      {
        ...
        $ticket->update(array('id' => $tick['id'],
                              'status'       => self::IN_ATTENDANCE,
                              '_auto_update' => true));
        
        if($tick['status'] == self::FEEDBACK)
        {
          $query_returnfeedback = "INSERT INTO `glpi_ticketfollowups` (`tickets_id`, `date`, `users_id`, `content`, `is_private`, `requesttypes_id`) VALUES ('".$tick['id']."', NOW(), 2, 'Feedback do usuário recebido, status do chamado alterado automaticamente de Aguardando Feedback para Em atendimento.', 0, 1); ";
          // EN //$query_returnfeedback = "INSERT INTO `glpi_ticketfollowups` (`tickets_id`, `date`, `users_id`, `content`, `is_private`, `requesttypes_id`) VALUES ('".$tick['id']."', NOW(), 2, 'Recebido feedback do usuário. Alterado automaticamente o status de Aguardando feedback para Em atendimento.', 0, 1); ";

          $DB->query($query_returnfeedback);
        } 
    ...
    */
      ...
      /* 
      // Inserir essa no banco de dados:
      INSERT IGNORE INTO `glpi_crontasks` (`itemtype`, `name`, `frequency`, `param`, `state`, `mode`, `allowmode`, `hourmin`, `hourmax`, `logs_lifetime`, `lastrun`, `lastcode`, `comment`) VALUES ('Ticket', 'returnticketfeedback', 300, NULL, 1, 1, 3, 0, 24, 10, '2015-12-23 12:46:00', NULL, NULL);
      */
   }
